...
void create_new_filename(char *input_file_name, char *output_file_name) {
    ...
    int after_first_slash_id = 0;
    for (int i = input_len; i > -1; i--) {
        if (input_file_name[i] == '/') {
            after_first_slash_id = i + 1;
            break;
        }
    }
    strncpy(output_file_name, input_file_name, after_first_slash_id);

    output_file_name[after_first_slash_id] = '\0';

    strcat(output_file_name, "out_");

    strcat(output_file_name, input_file_name + after_first_slash_id);
